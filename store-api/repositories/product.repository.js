...
async function updateProduct(product){
    const conn = await connect();
    try {
        const sql = "UPDATE products SET name = $2, description = $3, value = $4, stock = $5, supplier_id = $6 WHERE product_id = $1 RETURNING *";
        const values = [product.product_id, product.name, product.description, product.value, product.stock, product.supplier_id];
        const res = await conn.query(sql, values);
        return res.rows[0];
    } catch (err) {
        throw err;
